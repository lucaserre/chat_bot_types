...
Serviços de nuvem, LLMs, APIs. Esses são os principais recursos utilizados atualmente por diversas áreas da tecnologia, indo do front-end ao back-end. 
...
Nesse processo, percebi que, embora existam ferramentas conceituais muito antigas, as novas tecnologias vieram para aprimorar e escalar o que já existia
...
finir o que seria o foco. Através de um questionário consegui definir que o local possui 3 problemas específicos: buracos nas ruas (que demoravam para serem resolvidos), descarte incorreto de lixo (lixeiras lotadas) e de som (motos, vizinhos barulhentos), 
sendo definido do com mais reclamações, para o com menos reclamações.  

Posteriormente fiz outra pesquisa para verificar como esses problemas afetavam e como ele eram tratados pela comunidade, além de também verificar se as pessoas estariam de acordo com uma possível solução. A seguir uma foto dos resultados: 
...
Para ser algo gratuito e de uso para a própria comunidade, busquei soluções que não exigiam pagar por serviços. Assim, cheguei no telegram, que disponibiliza a criação de um bot, o qual só precisaremos configurar. Além disso, utilizei outras ferramentas como o Ngrok e bibliotecas nativas e externas do Python (como SQLite e Flask) para estruturar o back-end.
...
## Segunda Parte (Whatsapp + Python)
  ...
  Nesse momento foi onde enxerguei do potencial que seria isso dentro de uma organização maior, como seriam as regras de negócio, todos os algoritmos, e toda a engenharia de software em si. Mas, obviamente, teria que ser dentro de um dos apps.
...
## Terceiro Momento (Whatsapp + Python + AWS)
  ...
  Por fim, quis trazer para um momento atual, onde a inteligência artificial está sendo aplicada em diversos setores das empresas, seja financeiro, para atendimento ao cliente com informações sobre sua conta, seja sobre processos de pagamento dentro de uma empresa,
...
'''
    ...
    Você deve atender a todos os clientes de maneira cordial, mesmo que te chamem/respondam com gírias, prezando por respostas pragmáticas e que não fujam do escopo do qual você é responsável, ou seja, a parte financeira.
    
    Além disso, é zeloso pela segurança de informações. Apesar de a pessoa estar validada na base de dados, ela não tem, nem pode ter, acesso a informações de outros clientes além do CPF cadastrado para o contrato específico.
    
    Você deve realizar uma análise contundente de como o cliente está respondendo (pontuando entre muito bravo, chateado, desconfortável, de acordo, ou feliz) e responder com base nesse sentimento. Caso ele esteja entre muito bravo e desconfortável, você deve utilizar palavras que tranquilizem o cliente.
    
    A regra de desconto se baseará na forma e na data em que o cliente realizará o pagamento. Se houver somente uma parcela com até 3 dias de atraso, caso o cliente solicite, você pode retirar os juros e multa por atraso para pagamentos através do PIX; caso contrário, mantenha. Caso ultrapasse os 3 dias ou tenha duas ou mais parcelas em atraso, não é permitida a retirada dos juros. Nesse caso, é necessário um atendimento humano para negociação, e você perguntará ao cliente se ele deseja ser transferido para verificar a possibilidade de isenção.
    
    Sempre pergunte se o cliente precisa de mais alguma ajuda. Caso ele indique que o atendimento pode ser encerrado, despeça-se educadamente e adicione OBRIGATORIAMENTE a palavra-chave [FIM_ATENDIMENTO] no final da sua resposta. 
    
    Dados do cliente: {json.dumps(info_cliente, ensure_ascii=False)}'''
